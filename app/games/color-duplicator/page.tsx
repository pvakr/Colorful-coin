...
export default function ColorDuplicator() {
  ...
  }, [timeLeft, status, isFeedbackOpen])
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-2xl">
                {status === "win" ? <CheckCircle className="w-6 h-6" /> : <XCircle className="w-6 h-6" />}
                {status === "win" ? "Correct!" : "Wrong!"}
              </DialogTitle>
              <DialogDescription>
                {status === "win" ? "+1 score" : "Try the next round"}
              </DialogDescription>
            </DialogHeader>
